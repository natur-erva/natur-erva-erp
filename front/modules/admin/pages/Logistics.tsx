    ...
    <PageShell
      ...
      actions={
        <button onClick={() => load(true)} disabled={refreshing}
          className="flex items-center gap-2 px-3 py-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
          <RefreshCw className={`w-4 h-4 ${refreshing ? 'animate-spin' : ''}`} /> Atualizar
        </button>
      }
    >
      {/* Pipeline visual */}
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 mb-6 overflow-x-auto">
        <div className="flex items-center min-w-max gap-0">
            ...
            return (
              <React.Fragment key={key}>
                <button
                  ...
                  className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-lg transition-all ${isActive ? `${s.bg} ring-2 ${s.ring}` : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
                >
                ...
                {idx < PIPELINE.length - 1 && (
                  <div className="flex-1 h-0.5 bg-gray-200 dark:bg-gray-700 mx-1 min-w-[16px]" />
                )}
              </React.Fragment>
            );
          })}
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-lg transition-all ${filterStatus === 'delivered' ? 'bg-green-50 dark:bg-green-900/30 ring-2 ring-green-400' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-lg transition-all ${filterStatus === 'completed' ? 'bg-emerald-50 dark:bg-emerald-900/30 ring-2 ring-emerald-500' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1.5 px-4 py-2 rounded-lg transition-all ${filterStatus === 'cancelled' ? 'bg-red-50 dark:bg-red-900/30 ring-2 ring-red-400' : 'hover:bg-gray-50 dark:hover:bg-gray-700'}`}
          >
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full pl-9 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-green-500/40"
        />
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {filtered.map(order => {
            ...
            return (
              <div key={order.id} className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 shadow-sm">
                {/* Progress bar mini */}
                {pipelineIdx >= 0 && (
                  <div className="flex h-1">
                    {PIPELINE.map((_, i) => (
                      <div key={i} className={`flex-1 ${i <= pipelineIdx ? 'bg-green-500' : 'bg-gray-100 dark:bg-gray-700'}`} />
                    ))}
                  </div>
                )}
                ...
                <div className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-x-2 gap-y-0.5">
                      <span className="font-semibold text-sm text-gray-900 dark:text-white">{order.customerName}</span>
                      {order.trackingCode && (
                        <span className="text-xs font-mono text-gray-400">{order.trackingCode}</span>
                      )}
                    </div>
                ...
                {/* Detalhes expandidos */}
                {isExpanded && (
                  <div className="border-t border-gray-100 dark:border-gray-700 px-4 py-3 bg-gray-50 dark:bg-gray-900/30 space-y-3">
                    {/* Endereço */}
                    ...
                    {/* Notas */}
                    {order.notes && (
                      <div className="flex items-start gap-2 text-sm text-gray-500 dark:text-gray-400 italic">
                    ...
                    <div className="space-y-1">
                      {order.items.map((item, i) => (
                        <div key={i} className="flex items-center justify-between text-sm">
                          ...
                          <span className="text-gray-500 font-medium bg-gray-100 dark:bg-gray-700 px-2 py-0.5 rounded text-xs">×{item.quantity}</span>
                        </div>
                      ))}
                    </div>
                  </div>
