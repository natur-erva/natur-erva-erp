        ...
        {([
          ...
          <button
            ...
            className={`flex-1 py-2 px-3 rounded-lg text-sm font-medium transition-colors flex items-center justify-center gap-2 ${
              tab === key ? 'bg-white dark:bg-gray-900 text-gray-900 dark:text-white shadow-sm' : 'text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white'
            }`}
          >
            {label}
            {badge ? (
        ...
        <>
          ...
          {tab === 'affiliates' && (
            <div className="space-y-3">
              ...
              {affiliates.map(aff => (
                <div key={aff.id} className="bg-white dark:bg-gray-900 rounded-2xl p-5 border border-gray-200 dark:border-gray-800 shadow-sm">
                  <div className="flex items-start justify-between gap-3 mb-3">
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{aff.email}{aff.phone ? ` · ${aff.phone}` : ''}</p>
                    </div>
                    <button
                  ...
                  <p className="text-xs text-gray-400 mt-2">Taxa: {Number(aff.commission_rate).toFixed(0)}% · Desde {fmt(aff.created_at)}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {tab === 'commissions' && (
            <div className="space-y-3">
              ...
              {commissions.map(c => (
                <div key={c.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                  <button
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-gray-900 dark:text-white text-sm">{c.affiliate_name}</span>
                    ...
                    <div className="text-right flex-shrink-0 ml-3">
                      ...
                      <p className="text-xs text-gray-400">{Number(c.commission_rate).toFixed(0)}% de {Number(c.order_amount).toFixed(2)} MT</p>
                      {c.status === 'pending' && (expandedId === c.id ? <ChevronUp className="w-4 h-4 text-gray-400 ml-auto mt-1" /> : <ChevronDown className="w-4 h-4 text-gray-400 ml-auto mt-1" />)}
                    </div>
                  </button>
                  {expandedId === c.id && c.status === 'pending' && (
                    <div className="px-5 pb-4 space-y-3 border-t border-gray-100 dark:border-gray-800 pt-3">
                      <textarea
                        ...
                        className="w-full px-3 py-2 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm text-gray-900 dark:text-white resize-none focus:outline-none focus:ring-2 focus:ring-green-500"
                      />
          ...
          {tab === 'withdrawals' && (
            <div className="space-y-3">
              ...
              {withdrawals.map(w => (
                <div key={w.id} className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 shadow-sm overflow-hidden">
                  <button
                    className="w-full px-5 py-4 flex items-center justify-between text-left hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
                  ...
                  >
                    <div className="min-w-0">
                      <div className="flex items-center gap-2 flex-wrap">
                        <span className="font-medium text-gray-900 dark:text-white text-sm">{w.affiliate_name}</span>
                    ...
                    <div className="text-right flex-shrink-0 ml-3">
                      <p className="text-base font-bold text-gray-900 dark:text-white">{Number(w.amount).toFixed(2)} MT</p>
                      {w.status === 'pending' && (expandedId === w.id ? <ChevronUp className="w-4 h-4 text-gray-400 ml-auto mt-1" /> : <ChevronDown className="w-4 h-4 text-gray-400 ml-auto mt-1" />)}
                    </div>
                  </button>
                    ...
                    <div className="px-5 pb-4 space-y-3 border-t border-gray-100 dark:border-gray-800 pt-3">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-medium flex items-center justify-center gap-1.5 transition-colors disabled:opacity-50"
                        >
