    ...
    >
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-start justify-center p-4 overflow-y-auto">
          <div className="bg-white dark:bg-gray-900 rounded-2xl w-full max-w-3xl my-8 shadow-2xl border border-gray-200 dark:border-gray-700">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  {coverPreview ? (
                    <div className="relative w-32 h-20 rounded-xl overflow-hidden border border-gray-200 dark:border-gray-700 flex-shrink-0">
                    ...
                    <div className="w-32 h-20 rounded-xl border-2 border-dashed border-gray-300 dark:border-gray-700 flex items-center justify-center flex-shrink-0">
                      <Image className="w-6 h-6 text-gray-400" />
                    </div>
                  )}
                  <label className="flex items-center gap-2 px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg cursor-pointer hover:border-brand-500 text-sm text-gray-500 transition-colors">
              ...
              <div>
                ...
                <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500"
              ...
              <div>
                ...
                <textarea value={form.summary} onChange={e => setForm(f => ({ ...f, summary: e.target.value }))} rows={2}
                  className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 resize-none"
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-700">
              <button onClick={() => setShowForm(false)} className="px-4 py-2 text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg text-sm transition-colors">
                Cancelar
              </button>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {posts.map(post => (
            <div key={post.id} className="bg-surface-overlay rounded-xl border border-border-default p-4 flex items-center gap-4">
              {post.coverImage ? (
                <img src={post.coverImage} alt="" className="w-16 h-16 rounded-lg object-cover flex-shrink-0 border border-border-default" />
              ) : (
                <div className="w-16 h-16 rounded-lg bg-surface-raised flex items-center justify-center flex-shrink-0">
                  <FileText className="w-6 h-6 text-content-muted opacity-40" />
                </div>
              )}
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-0.5">
                  <span className="font-semibold text-content-primary truncate">{post.title}</span>
                  <span className={`px-2 py-0.5 rounded text-xs font-medium ${post.status === 'published' ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
                    {post.status === 'published' ? 'Publicado' : 'Rascunho'}
                  </span>
                </div>
                {post.summary && <p className="text-sm text-content-secondary truncate">{post.summary}</p>}
              ...
              <div className="flex items-center gap-2 shrink-0">
                <a href={`/blog/${post.slug}`} target="_blank" rel="noopener noreferrer"
                  className="p-2 rounded-lg hover:bg-surface-raised text-content-muted hover:text-brand-600 transition-colors" title="Ver no site">
                  <Globe className="w-4 h-4" />
                </a>
                <button onClick={() => openEdit(post)} className="p-2 rounded-lg hover:bg-blue-50 dark:hover:bg-blue-900/20 text-blue-600 dark:text-blue-400 transition-colors">
