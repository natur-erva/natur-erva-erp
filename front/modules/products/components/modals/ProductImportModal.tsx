      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Importar Produtos via Planilha</h2>
        ...
        {/* Steps indicator */}
        <div className="flex items-center gap-2 px-6 pt-4 text-xs text-gray-500 dark:text-gray-400">
          {(['upload', 'preview', 'results'] as Step[]).map((s, i) => (
            <React.Fragment key={s}>
              <span className={`px-2 py-0.5 rounded-full font-medium ${step === s ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400' : 'text-gray-400'}`}>
        ...
        <div className="flex-1 overflow-y-auto p-6">
          ...
          {step === 'upload' && (
            <div className="space-y-6">
              ...
              {/* Upload zone */}
              <div>
                <h3 className="font-semibold text-gray-800 dark:text-white mb-3">2. Carregue a Planilha Preenchida</h3>
                <div
                ...
                >
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Suporta .xlsx</p>
                  <input
                    ref={fileRef}
                    type="file"
                    accept=".xlsx,.xls"
              ...
              <div className="bg-gray-50 dark:bg-gray-800 rounded-xl p-4 text-sm text-gray-600 dark:text-gray-400 space-y-1">
                <p className="font-medium text-gray-700 dark:text-gray-300 mb-2">Regras importantes:</p>
                <p>• A coluna <code className="bg-gray-200 dark:bg-gray-700 px-1 rounded">nome</code> é obrigatória em todas as linhas.</p>
                <p>• Para produto <strong>fixo</strong>: deixe as colunas <code className="bg-gray-200 dark:bg-gray-700 px-1 rounded">variante_*</code> em branco.</p>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-4 text-sm">
                ...
                <button
                  ...
                  className="ml-auto text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 underline text-xs"
                >
                  ...
                  <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                    {rows.slice(0, 50).map((row, i) => (
                      <tr key={i} className={i % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50/50 dark:bg-gray-800/50'}>
                        <td className="px-3 py-1.5 font-medium text-gray-900 dark:text-white">{row.nome}</td>
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-400">{row.categoria || '—'}</td>
                        <td className="px-3 py-1.5 text-gray-600 dark:text-gray-400">{row.unidade || 'un'}</td>
          ...
          {step === 'results' && result && (
            <div className="space-y-4">
              ...
              {result.errors.length > 0 && (
                <div>
                  <h4 className="font-semibold text-gray-700 dark:text-gray-300 mb-2 text-sm">Detalhes:</h4>
                  <div className="max-h-48 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-700">
                    {result.errors.map((e, i) => (
                      <div key={i} className="flex items-start gap-2 px-4 py-2 border-b border-gray-100 dark:border-gray-700 last:border-0">
                        <AlertCircle className="w-4 h-4 text-yellow-500 mt-0.5 flex-shrink-0" />
                        <div className="text-xs">
                          <span className="font-medium text-gray-800 dark:text-gray-200">{e.nome}</span>
                          <span className="text-gray-500 dark:text-gray-400 ml-2">— {e.reason}</span>
                        </div>
                      </div>
                    ))}
                  </div>
